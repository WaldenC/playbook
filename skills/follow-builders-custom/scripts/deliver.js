  ...
  try {
    switch (delivery.method) {
      ...
      case 'email': {
        ...
        // Support delivery.email (string) or delivery.emails (array)
        const toEmail = delivery.emails || delivery.email;
        if (!apiKey) throw new Error('RESEND_API_KEY not found in .env');
        if (!toEmail) throw new Error('delivery.email or delivery.emails not found in config.json');
        await sendEmail(digestText, apiKey, toEmail);
        const label = Array.isArray(toEmail) ? toEmail.join(', ') : toEmail;
        console.log(JSON.stringify({
          status: 'ok',
          method: 'email',
          message: `Digest sent to ${label}`
        }));
        break;
      }
